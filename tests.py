...
class TestComments(TestCase):
    ...
    def test_items_in_list(self):
        ...
        self.assertIn(self._comment_fragment(), response.content.decode('UTF-8'))
        ...
        self.assertNotIn(self._no_comments_message_fragement(), response.content.decode('UTF-8'))
        self.assertNotIn(self._required_message_fragement(), response.content.decode('UTF-8'))

    def test_no_items_in_list(self):
        response = self.c.get('/')

        self.assertEqual(response.status_code, 200)
        self.assertNotIn(self._comment_fragment(), response.content.decode('UTF-8'))
        self.assertIn(self._no_comments_message_fragement(), response.content.decode('UTF-8'))
        self.assertIn(self._form_fragement(), response.content.decode('UTF-8'))
        self.assertNotIn(self._required_message_fragement(), response.content.decode('UTF-8'))
    ...
    def test_add_first_comment(self):
        ...
        response = self.c.post(location, {"name": "Don", "message": "message from test"})
    ...
    def test_incorrect_form(self):
        ...
        response = self.c.post("/", {"name": "", "message": "message from test"})
        self.assertEqual(response.status_code, 200)
        self.assertNotIn("<pre", response.content.decode('UTF-8'))
        self.assertIn("<form", response.content.decode('UTF-8'))
        self.assertIn(self._required_message_fragement(), response.content.decode('UTF-8'))
